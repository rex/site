...
NewSchema["static"]('createFromITunesAlbum', function(model, callback) {
  var album, disc_count, groups, new_item, tracks;
  if (callback == null) {
    callback = function() {};
  }
  new_item = new this();
  groups = _.groupBy(model.results, function(result) {
    return result.wrapperType;
  });
  album = _.first(groups.collection);
  disc_count = _.first(groups.track).discCount;
  tracks = _.map(groups.track, function(track) {
    return {
      ...
      duration: Math.floor(parseInt(track.trackTimeMillis / 1000)),
