var Env, async, debug, logger, mongo, _;
...
module.exports = function(after_env_vars_saved) {
  var credentials;
  if (after_env_vars_saved == null) {
    after_env_vars_saved = function() {};
  }
  credentials = {
  ...
  return async.each(_.keys(credentials), function(key, next) {
    return Env.findOneAndUpdate({
      key: key
    }, {
      $set: {
        key: key,
    ...
    if (debug) {
      if (err) {
        ...
        logger("Loaded " + (Object.keys(credentials).length) + " credentials into database");
      }
    }
    return after_env_vars_saved();
  });
};
